...
var RoutingProxy = exports.RoutingProxy = function (options) {
  ...
  this.emulateModProxy = options.emulateModProxy || false;
  this.changeOrigin = options.changeOrigin || this.emulateModProxy;
  this.rewriteRedirects = options.rewriteRedirects || this.emulateModProxy;
  ...
  this.on('newListener', function (evt) {
    if (evt === 'proxyError' || evt === 'webSocketProxyError' || evt === 'start') {
...
RoutingProxy.prototype.add = function (options) {
  ...
  //
  ['https', 'enable', 'forward', 'emulateModProxy', 'changeOrigin', 'rewriteRedirects'].forEach(function (key) {
...
RoutingProxy.prototype.proxyRequest = function (req, res, options) {
  ...
  if (this.proxyTable && !options.host) {
    ...
    // then emit `routenotfound` event if present, otherwise
    // respond with `404` since we do not have a valid proxy target.
    //
    if (!location) {
      this._onRouteNotFound(req, res);
      return;
    }
  ...
  if (this.emulateModProxy && this.proxyTable && !options.source) {
    options.source = location.source;
  }
...
//
// ### @private function _onRouteNotFound (req, res)
// #### @req {ServerRequest} Incoming HTTP Request to proxy.
// #### @res {ServerResponse} Outgoing HTTP Request to write proxied data to.
// Emits `routenotfound` event to give the caller a chance to react to non
// defined routes. If no listner is found, respond with `404`
//
RoutingProxy.prototype._onRouteNotFound = function(req, res) {
  if(this.listeners('routenotfound').length == 0) {
    this.tryEndResponseWithNotFound(res);
    return;
  }
...
//
// ### @private function tryEndResponseWithNotFound (res)
// #### @res {ServerResponse} Outgoing HTTP Request to write proxied data to.
// Respond with `404` since we do not have a valid proxy target
//
RoutingProxy.prototype.tryEndResponseWithNotFound = function(res) {
  try {
    res.writeHead(404);
    res.end();
  }
